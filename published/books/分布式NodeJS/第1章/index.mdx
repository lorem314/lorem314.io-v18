...
import { Note, Tip, Border } from '@/components/elements/oreilly'
...
<Figure id="1-2" title="连续与并行 IO 的可视化" src="/books/分布式NodeJS/第1章/图片1-2_连续与并行IO的可视化.png" />
...
<Figure id="1-3" title="NodeJS 的层级" src="/books/分布式NodeJS/第1章/图片1-3_NodeJS的层级.png" />
...
<blockquote>
  每个阶段结束时，在进入下一个阶段之前，都会检查两个微队列中是否有任务，有的话会处理完任务，再去下一个阶段。如果在 Promise 回调中又通过 process.nextTick 和 Promise 添加了各自的回调函数，则 Promise 会将所有自身的回调处理完之后，在进入下一个阶段之前，继续处理 process.nextTick 中的回调，在检查有无自身的回调，确保微队列中没有任务后，再去往下个阶段。
...
<Figure id="1-4" title="NodeJS 事件循环的不同阶段" src="/books/分布式NodeJS/第1章/图片1-4_NodeJS事件循环的不同阶段.png" />
...
<Figure id="1-5" title="web-api 和 recipe-api 之间的关系" src="/books/分布式NodeJS/第1章/图片1-5_web-api和recipe-api之间的关系.png" />
